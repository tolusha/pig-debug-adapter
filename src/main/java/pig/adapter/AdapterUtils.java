...
public class AdapterUtils {
    ...
    /**
     * Convert the source platform's line number to the target platform's line
     * number.
     *
     * @param line                the line number from the source platform
     * @param sourceLinesStartAt1 the source platform's line starts at 1 or not
     * @param targetLinesStartAt1 the target platform's line starts at 1 or not
    ...
    /**
     * Convert the source platform's path format to the target platform's path
     * format.
     *
     * @param path        the path value from the source platform
     * @param sourceIsUri the path format of the source platform is uri or not
     * @param targetIsUri the path format of the target platform is uri or not
    ...
    /**
     * Convert a file uri to a file path, or null if this uri does not represent a
     ...
     * @throws MalformedURLException if the file path cannot be constructed to an
     *                               url because of some errors.
     */
    ...
    /**
     * Populate the response body with the given error message, and mark the success
     * flag to false. At last return the response object back.
     *
    ...
    /**
     * Populate the response body with the given exception, and mark the success
     * flag to false. At last return the response object back.
     *
     * @param response  the response object
     * @param errorCode the error code
     ...
     */
    public static CompletableFuture<Response> createAsyncErrorResponse(Response response, ErrorCode errorCode,
            String errorMessage) {
    ...
    /**
     * Indicates that line isn't empty and isn't a comment.
     */
    public static boolean isQuery(String line) {
        String trimmed = line.trim();
        return !trimmed.isEmpty() && !trimmed.trim().startsWith("--");
    }

    public static List<Tuple> readValue(PigServer server, String alias) throws IOException {
        Iterator<Tuple> iterator = server.openIterator(alias);
